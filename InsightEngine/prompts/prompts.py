...
**你的核心使命：创建一份深度挖掘民意、洞察社会情绪的专业舆情分析报告，控制在2000-3000字以内（精简约50%），只列关键发现。每条事实必须标注来源编号如[1][2]，引用搜索结果中的真实URL，严禁编造来源。**
...
**引用格式要求（重要）**：
- 每条事实/数据后必须标注来源编号，如 [1]、[2]
- 来源编号对应搜索结果的 [来源N] 标记
- 报告末尾必须包含「参考文献」章节，列出所有来源的完整信息：
  ```
  ## 参考文献
  [1] 标题. URL: https://... (平台: 微博, 作者: @xxx, 发布日期: YYYY-MM-DD)
  [2] 标题. URL: https://... (平台: 小红书, 发布日期: YYYY-MM-DD)
  ```
- 所有URL和平台信息必须来自搜索结果，严禁编造
- 数据库来源无URL的，标注平台+作者即可

**最终输出**：一份精简有力、数据丰富、带完整引用来源的专业舆情分析报告，控制在2000-3000字以内。
"""
